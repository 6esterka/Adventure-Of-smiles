...
#include "Menu_of_the_game.h"
using namespace std;
///////////////////////////
//Constants
const int maxUnits = 50;
const int initial_row = 40;
const int initial_col = 40;
////////////////////////
...
void main() {
	...
	if (exit_button == false) {
		...
		SetupSystem();
		PlaySoundA("Pygmalion_m_-_Arachnodactylia.wav", NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		...
		fread_s(rank_list,rank_num*sizeof(int),sizeof(int), 10, file);
		fclose(file);
		file = nullptr;
		//Writing the coins into high score
		for (int i = 0; i < rank_num; ++i) {
			if (rank_list[i] < coins&&rank_list[i]!=0) {
			...
			else if (rank_list[i] < coins&&rank_list[i] == 0) {
				rank_list[i] = coins;
				break;
			}
		}
		
		fopen_s(&file, "highscore.bin", "wb");
		fwrite(rank_list,sizeof(int),10,file);
		fclose(file);
		Shutdown(checkWin);
	}
